...
def evaluate(gold_ud, system_ud):
    ...
    # Modified to output diagnostics
    def spans_score(gold_spans, system_spans, gold_text, system_text):
        correct, gi, si = 0, 0, 0
        while gi < len(gold_spans) and si < len(system_spans):
            if system_spans[si].start < gold_spans[gi].start:
                # print("Unexpected predicted token '" + ''.join(
                #    system_text[system_spans[si].start:system_spans[si].end]) + "' in '" + ''.join(
                #    system_text[(system_spans[si].start - 10):(system_spans[si].end + 10)]) + "'")
                si += 1
            elif gold_spans[gi].start < system_spans[si].start:
                # print("Unexpected gold token '" + ''.join(
                #    gold_text[gold_spans[gi].start:gold_spans[gi].end]) + "' in '" + ''.join(
                #    gold_text[(gold_spans[gi].start - 10):(gold_spans[gi].end + 10)]) + "'")
                gi += 1
            else:
    ...
    def align_words(gold_words, system_words, gold_text, system_text):
        ...
        while gi < len(gold_words) and si < len(system_words):
            if gold_words[gi].is_multiword or system_words[si].is_multiword:
                ...
                if (gold_words[gi].span.start, gold_words[gi].span.end) == (
                    ...
                    #print("Unexpected gold word '" + ''.join(
                    #    gold_text[gold_words[gi].span.start:gold_words[gi].span.end]) + "' in '" + ''.join(
                    #    gold_text[(gold_words[gi].span.start - 10):(gold_words[gi].span.end + 10)]) + "'")
                    gi += 1
                else:
                    #print("Unexpected predicted word '" + ''.join(
                    #    system_text[system_words[si].span.start:system_words[si].span.end]) + "' in '" + ''.join(
                    #    system_text[(system_words[si].span.start - 10):(system_words[si].span.end + 10)]) + "'")
                    si += 1
    ...
    # Align words
    alignment = align_words(gold_ud.words, system_ud.words, gold_ud.characters, system_ud.characters)
    
    # Compute the F1-scores
    return {
        "Tokens": spans_score(gold_ud.tokens, system_ud.tokens, gold_ud.characters, system_ud.characters),
        "Sentences": spans_score(gold_ud.sentences, system_ud.sentences, gold_ud.characters, system_ud.characters),
